...
class FormTemplate(Document):
	...
	if TYPE_CHECKING:
		from form_printer.form_printer.doctype.form_template_prompts.form_template_prompts import FormTemplatePrompts
...
def convert_pdf_to_image(form_template_id):
    ...
    file = frappe.db.get_value(
        "Form Template", form_template_id, "file")
    ...
    default_font = font_counter.most_common(
        1)[0][0] if font_counter else "helvetica"
    default_font_size = font_size_counter.most_common(
        1)[0][0] if font_size_counter else 12
    ...
    form_template = frappe.get_doc(
        "Form Template", form_template_id)
...
def create_form_fields_annotations(page, image_doc, image_url,font_counter, font_size_counter):
	...
	for field in fields:
		...
		field_doc = frappe.get_doc({
			...
			"value": "xywh=pixel:{},{},{},{}".format(x, y, w, h),
...
def return_standard_font(font):
    ...
    match font:
        case "courier" | "courier-bold" | "courier-boldoblique" | "courier-oblique" | 'cour' | 'cobo' | 'cobi' | 'coit':
            return "courier"
        case "helvetica" | "helvetica-bold" | "helvetica-boldoblique" | "helvetica-oblique" | 'helv' | 'heit' | 'hebo' | 'hebi':
            return "helvetica"
        case "times-roman" | "times-bold" | "times-bolditalic" | "times-italic" | 'tiro' | 'tibo' | 'tiit' | 'tibi':
            return "times-roman"
        case "symbol" | 'symb':
            return "symbol"
...
def get_form_template_prompts(form_template_id):
	...
	form_template_fields = frappe.get_all("Form Template Field", filters=[["form_template", "=", form_template_id], ["value_type", "=", "Prompt"]])
	...
	for field in form_template_fields:
		prompt_field = next(
			(obj for obj in prompts if obj.field_name == field['field_value']), None)
		if prompt_field:
