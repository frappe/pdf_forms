...
removed_column_type = ["Attach", "Attach Image", "Autocomplete", 'Column Break', "Button", "Barcode", "HTML", "Fold", "Geolocation",
                       "Heading", "Icon", "Section Break", 'Tab Break', "Signature", "Color", "HTML Editor", "Image", "Markdown Editor"]
...
def get_json_schema_for_doctype(doctype):
    ...
    fields = [field for field in fields if field.get(
        'fieldtype') not in removed_column_type]
...
def map_frappe_fields_to_json_schema(fields, doctype=None):
    ...
    for field in fields:
        ...
        if field.get('fieldtype') == 'Table' and field.get('options'):
            ...
            # Remove fields with column type
            child_fields = [child_field for child_field in child_fields if child_field.get(
                'fieldtype') not in removed_column_type]
            ...
            json_field['items'] = {
                'schema_type': 'object',
                'description': field.get('label') or field.get('fieldname'),
                'properties': map_frappe_fields_to_json_schema(child_fields, child_doc_meta.get("name", None))['properties'],
                'required': map_frappe_fields_to_json_schema(child_fields, child_doc_meta.get("name", None))['required']
            }
    ...
    for app in install_apps:
        ...
        if 'form_printer_data_source' in app_hooks:
            ...
            for key, value in data.items():
                if key == source:
                    found = True
                    return value.get('get_meta')[0] if type(value.get('get_meta')) == list else value.get('get_meta')
    if not found:
        return frappe.throw(
            f"Source {source} not found in any of the installed apps")
...
def execute_function_from_path(api_path, *args, **kwargs):
    ...
    valid_kwargs = {
        key: value for key, value in kwargs.items() if key in func_signature.parameters
    }
...
def generate_excel_from_json_schema(json_schema):
    ...
    def process_schema(schema, sheet_name):
        ...
        for key, field in schema.get("properties", {}).items():
            ...
            if field.get("schema_type") == "array" and "items" in field and "properties" in field["items"]:
                ...
                child_sheet_name = field.get(
                    "description", key).strip() or f"Sheet_{key}"
                ...
                table_sheets[child_sheet_name] = process_schema(
                    child_schema, child_sheet_name)
    ...
    def write_sheet(sheet_name, data):
        ws = wb.create_sheet(
            title=sheet_name) if sheet_name != "Main" else wb.active
        ws.title = sheet_name
        ...
        # Add rows
        for row in data:
            ws.append([row["key"], row["description"],
                      row["fieldtype"], row["options"], row["doctype"]])
