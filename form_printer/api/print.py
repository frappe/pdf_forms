...
def print_form_template(template_id, data, print_name = None, print_type='pdf'):
    ...
    for i in range(len(doc)):
        ...
        if i in page_index_include_in_images:
            image = [image for image in repeated_images if image['page_index'] == i][0]
            ...
            for copy_num in range(copies):
                ...
                for widget in page.widgets():
                    ...
                    new_widget = temp_page.add_widget(
                        widget)

                    xref_map[str(widget.xref)] = str(new_widget.xref)

                # Apply the data print logic to the copied page
                annotate_form_template(temp_page, i, template_id, data,
                         font, font_size, base_index=(base_index * (copy_num + 1)), xref_map=xref_map)
...
def fetch_repeated_document_template_images(template_id):
    return frappe.get_all('Form Template Image', filters={'form_template_id': template_id, 'repeat_page': 1}, fields=['name', 'page_index', 'repeat_after', 'copies', 'base_index'], order_by="page_index asc")


def annotate_form_template(page, i, template_id,  data, font, font_size, base_index=0, xref_map={}):
    ...
    annotatate_auto_fields(page, auto_annotations, data,
                           font, font_size, base_index, xref_map)
    ...
    annotatate_manual_fields(
        page, manual_annotations, data, font, font_size, base_index)

def annotatate_auto_fields(page, auto_annotations, data, font, font_size, base_index=0, xref_map={}):
    ...
    for field in fields:
        ...
        annotation = get_annotation(
            field.field_name, auto_annotations, field.xref, xref_map)

        if annotation:
            # get the field value from the data
            if annotation.is_default_jinja:
                default_value = frappe.render_template(
                    annotation.default_value, data)
            else:
                default_value = annotation.default_value

            value = get_field_value(
                annotation, data, base_index)

            if isinstance(value, str):
                value = value.strip()

            value = value if (value != None and value !=
                              '' and value != 'None') else default_value
            ...
            if value != None:
                ...
                if annotation.field_type == 'Text':
                    ...
                    field.text_fontsize = annotation.font_size if annotation.override_style and annotation.font_size > 0 else font_size
                    ...
                    field.field_value = str(
                        value) if value != None and value != 'None' else ''
                    field.update()
...
def annotatate_manual_fields(page, manual_annotations, data, font, font_size, base_index=0):
    ...
    for annotation in manual_annotations:
        ...
        if value != None:
            ...
            width = frappe.get_value(
                'Form Template Image', annotation.form_template_image, 'width')
            ...
            widget.text_fontsize = annotation.font_size if annotation.override_style and annotation.font_size > 0 else font_size
            text_font = annotation.font if annotation.font and annotation.font != 'None' else font
            font_name = fitz.Font(text_font).name
...
def get_field_value(annotation, data, base_index):
    ...
    if annotation.value_type == 'Text':
        value = str(
            annotation.field_value) if annotation.field_value and annotation.field_value != 'None' else None
        return str(annotation.field_value) if annotation.field_value and annotation.field_value != 'None' else None
        ...
        return formatted_value if formatted_value != None else None
