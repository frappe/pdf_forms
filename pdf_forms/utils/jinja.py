...
def format_currency(value, currency=None, precision=None):
	...
	default_currency = frappe.db.get_default("currency") if currency is None else currency
	return (
		frappe.utils.fmt_money(amount=value, currency=default_currency, precision=precision)
...
def format_number(value, precision=None):
	...
	default_precision = frappe.db.get_default("float_precision") if precision is None else precision
	return (
		frappe.utils.fmt_money(amount=value, precision=cint(default_precision)) if value is not None else ""
	)
...
def format_currency_without_symbol(value, currency=None, precision=None):
	# format the currency in the system's currency format
	return frappe.utils.fmt_money(amount=value, precision=precision) if value is not None else ""
...
def get_address_object(doctype, docname):
	...
	address_ids = frappe.db.get_list(
		"Address",
		filters=[
